...
public class Gui extends Application {
    ...
    public void start(Stage stage) {
        ...
        //____
        listView = new ListView(controller);
        ...
        Scene scene1 = new Scene(listView.listView);
    ...
    //____
    // Update the ListView with the new history values
    public void updateListView(List<IMemento> history) {
        listView.getItems().clear();
        Map<String, IMemento> mementoMap = new HashMap<>();
        for (IMemento memento : history) {
            String mementoString = memento.toString();
            listView.getItems().add(mementoString);
            mementoMap.put(mementoString, memento);
        }
        listView.updateListView(listView.getItems(), mementoMap);
    }
}